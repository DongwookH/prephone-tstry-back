...
const REPLY_ANSWER_ANGLES = [
  ...
  "실제 손님 사례 한 장면을 짧게(문의 주신 분/개통해 드린 분 이야기 — 계정 본인이 겪은 것처럼 쓰지 말 것)",
...
const COPY_STYLES: CopyStyle[] = [
  ...
  },
  {
    id: "firstperson",
    name: "손님 사례 경험담형",
    desc: "손님이 겪은 일을 구체적 장면으로 풀어 공감·재현 욕구를 자극. (계정=업체 — 본인이 겪은 것처럼 쓰지 않는다)",
    hook: "손님이 겪은 구체적 상황 한 장면(시간·감정 포함)으로 연다. 도입부는 매번 다르게 — \"어제 문의 주신 분이 딱 이랬어요\" / \"지난주에 오신 손님, 폰이 갑자기 먹통 됐다더라고요\" / \"개통해 드리다 보면 이런 분들 진짜 많아요\" / \"오늘 상담한 케이스인데\" 등에서 회전(같은 문구 반복 금지). ⚠️ '제가/저도 겪었다'는 1인칭 피해 경험 금지.",
    close: "같은 상황인 독자가 자기 사례를 댓글로 쓰고 싶게 끝낸다. 마케팅 카피가 아니라 업체가 손님 썰 풀듯 자연스럽게.",
    basis: "marketing-psychology: 가용성 휴리스틱·유사성(liking)·연대(unity). humanizer: 구체적 디테일. 2026-07-20 운영자 지시: 경험담은 손님 시점.",
  },
  {
    id: "myth",
    name: "반전/오해 정정형",
...
const GOOD_ANGLE_EXEMPLARS: string[] = [
  "[공감 페인포인트] 요금 밀려서 정지된 폰, 그냥 버려야 하나 고민하는 분들 많은데 사실 살릴 방법이 있어요.",
  "[내 케이스 vs 너 케이스] KT 요금 미납이랑 KT 단말기 할부 미납은 완전 다른 얘기예요. 한쪽은 그냥 개통되고, 한쪽은 폰을 못 씁니다.",
  "[찬반 유발] 선불폰 쓰면 손해라던데, 저희 손님들 중엔 후불 쓸 때보다 통신비가 줄었다는 분이 많아요. 상황 따라 다른 것 같더라고요.",
  "[손님 사례 디테일] 어제 개통해 드린 분, 비대면으로 유심 신청하고 개통까지 딱 5분 걸렸어요. 매장에 안 오시고요.",
...
const AI_TELL_CHECKLIST: string[] = [
  ...
  "화자는 앤텔레콤(개통 업체) 계정 — 본인이 미납·정지·개통 거절을 겪은 1인칭 경험담('제가 퇴짜 맞았다', '지난달 제/내 폰이 정지됐다', 주어 생략형 포함)이 있으면 손님 사례 시점('어제 문의 주신 분이…')으로 재작성. 업체 1인칭('이런 문의 많이 받아요')은 그대로 둔다.",
  "한국어 번역투 제거 — '~에 대해/~를 통해' 남발은 조사로 직결, '가지고 있다'는 '~이 있다/~하다'로, 이중 피동('~되어진다')과 '~에 의해' 피동은 능동으로, '~할 수 있어요' 남발은 단언으로(가능 여부 사실 서술은 예외), '~한 것이다' 종결은 직접 종결로, 연결어미 뒤 쉼표('확인하고,')는 쉼표 제거.",
];
...
13. **한국어 번역투** (humanize-korean 분류 이식) — 다음은 AI 번역투의 강한 신호다:
   - "~에 대해/~에 있어/~를 통해" 남발 → 조사로 직결 ("요금제에 대해 알아보면" ✗ → "요금제 알아보면")
   - "가지고 있다" have 직역 → "장점을 가지고 있어요" ✗ → "장점이 있어요"
   - 이중 피동 "~되어진다/~지게 된다" ✗, "~에 의해" 피동 → 행위자를 주어로
   - "~할 수 있어요" 남발 → 사실은 단언으로 ("아낄 수 있어요" → "아껴요"). 단, 개통 가능/불가 같은 '가능 여부' 사실 서술은 그대로 둔다.
   - "~한 것이다/~다는 것이다" 종결, 추측 종결("~로 보인다") 남발 ✗
   - 연결어미(-고/-며/-지만) 바로 뒤 쉼표 ✗ ("확인하고, 신청하면" → "확인하고 신청하면")
   - "그/그녀/그것" 대명사 직역 → 생략하거나 명사로.

# 🎭 화자(계정) 정체성 — 절대 규칙 (2026-07-20 운영자 지시)
- 이 계정은 **앤텔레콤(선불폰 개통 서비스 업체)** 계정이다.
- **본인이 고객 피해 상황을 겪은 것처럼 쓰지 마라**: "제가 개통 거절당했어요", "지난달 제 폰이 정지됐거든요", "저도 요금 밀려서" 같은 1인칭 피해 경험담 금지. 실화가 아니고, 개통 업체 계정이 개통을 거절당했다는 건 모순이라 신뢰가 깨진다. (주어를 생략한 "지난달에 폰이 먹통 됐거든요"처럼 화자 본인 일로 읽히는 문장도 동일하게 금지.)
- 경험담 각도는 반드시 **손님 사례 시점**으로: "어제 개통 문의 주신 분이…", "손님 중에 이런 분 계셨어요", "지난주에 개통해 드린 분 사례인데" 식.
- 업체로서의 1인칭은 얼마든지 가능: "이런 문의 진짜 많이 받아요", "개통해 드리다 보면", "1,000건 넘게 하다 보니".
- 본문·셀프 댓글 모두 적용.
...
- "공식" / "본사" / "직영" — 우리는 "인증판매점". 자기 지칭·타사 지칭 모두 금지.
  대체: "편의점 공식 앱" ✗→"편의점 앱" ✓ / "공식 사이트" ✗→"홈페이지" ✓ / "직영점" ✗→"매장" ✓
...
# 출력 (JSON만, 코드펜스 X, 문자열 안 줄바꿈은 \\n으로 escape)
{
  ...
  return drafts
    ...
    .filter((d) => {
      ...
      // 페르소나 가드 — 계정(업체) 본인이 미납·거절을 겪은 1인칭 경험담이면 폐기.
      // 경험담은 손님 사례 시점만 허용 (2026-07-20 운영자 지시).
