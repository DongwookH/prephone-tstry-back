...
${tb.extra_rules}`
  ...
  // ── 번호유지 오정보 고위험 키워드 — 프레임 강제 ──
  // 정지·미납류 키워드는 검색 의도가 "번호 살리기"라 Gemini가 자꾸 그 프레임으로
  // 써서 사실 가드에 3/3 폐기됨 (2026-07-26: 'SKT발신정지' 하루 생성 10→8 원인).
...
## 🚨 두 가지 더 (2026-07-31 실측 — 이걸 어겨서 하루 8편이 폐기됨)

**① 작성 규칙을 본문에 옮겨 적지 마세요.**
위에 준 자료에는 "어떻게 쓰라"는 지침이 섞여 있습니다. 그건 당신에게 하는 말이지
독자에게 할 말이 아닙니다. 특히 **요금 표기 규칙을 문장으로 만들지 마세요.**
  ✗ **'확정가'·'확정 가격'이라는 말 자체를 쓰지 마세요.** 본문·소제목·표 제목·목차 전부.
     (실제 폐기: 표 제목 "요금제 확정가 안내" — 2026-08-06 '쿠팡유심개통')
  ✗ "단정 표기 가능한 확정 가격은 6종입니다"  ✗ "확정가 6가지 안내"
  ✗ 소제목에 "후킹 포인트", "클리셰" 같은 기획 용어
  ○ 표 제목은 "요금제 안내", "요금제 한눈에 보기"처럼 담백하게
  ○ 금액은 상품명과 함께: "선불 기본1은 12,100원, 선불 396은 39,600원이에요"

**② 우리를 "공식"이라 부르지 마세요.**
우리는 통신사가 아니라 그 통신사의 **온라인 판매점**입니다. 공식이 아닙니다.
  ✗ "○○ 공식 채널" ✗ "공식 신청 페이지" ✗ "공식 홈페이지" ✗ "공식 판매점/대리점"
  ○ "신청 페이지", "요금제 안내", "카카오톡 상담 채널"
  (정부기관·통신사를 가리키는 "공식"은 써도 됩니다 — 예: "과학기술정보통신부 공식 자료")

## 🚨 화자 시점 규칙 (위반 시 글 전체 폐기)
글을 쓰는 당신은 **개통을 도와주는 판매점**입니다. 개통이 막혀 곤란했던 당사자가 아닙니다.
- **"저는 / 저도 / 저처럼 / 제 명의로"**에 미납·연체·거절·정지 경험을 붙이지 마세요.
  ✗ "저는 통신비 연체 이력이 몇 번 있어서 개통이 번번이 거절되었어요" ← 실제 발행 사고
  ✗ "저도 처음엔 막막했는데"  ✗ "저처럼 미납 이력이 있는 분들"
- 겪은 이야기는 **손님 사례 시점**으로만 쓰세요.
  ○ "얼마 전 오신 손님도 연체 이력 때문에 거절당하셨는데, 5분 만에 개통해 드렸어요"
- 반대로 **도움 주는 역할**의 1인칭은 괜찮습니다: "오늘 제가 알려드릴게요" ○`;

  // ⚠️ 이 블록은 **키워드와 무관하게 항상** 주입한다.
  //    가드(findNumberKeepingClaims)는 늘 켜져 있는데 규칙만 조건부로 주면,
  //    조건 밖 키워드에서 모델이 "번호를 살리는 방법"을 써도 배운 적이 없어
  //    3회 재시도를 그대로 소진한다 (2026-08-04: '선불폰당일개통' 3/3 폐기).
  //    가드와 프롬프트 규칙은 반드시 짝으로 — 조건부로 갈리면 안 된다.
  const numberRiskBlock = `

## 🚨 번호 관련 서술 규칙 (오정보 위험 최고 — 위반 시 글 전체 폐기)
...
- 자동 검열기가 "번호" 주변(±14자)에 '그대로/유지/살리다/지키다'류 단어가 보이면 **부정문·질문이어도 무조건 폐기**합니다.
  → 아래는 **내용이 맞는데도 폐기된 실제 문장들**입니다. 이렇게 쓰지 마세요:
  ✗ "직권해지 상태에서는 기존 번호를 유지할 수 없어요"
  ✗ "기존 번호를 되살릴 수 없어요"
  ✗ "신속하게 번호를 살리는 방법"  ✗ "원하는 번호를 곧바로 살리는 핵심 흐름"
  ○ "직권해지 이후에는 **새 번호로 발급**됩니다"  ○ "**새 번호**를 받아 바로 쓸 수 있어요"
- 번호는 오직 이런 형태로만 언급: **"번호는 새로 발급받아요"**, **"새 번호로 5분 만에 개통"**.
- **'유지·그대로·살리·지키' 네 어근을 '번호'와 같은 문장에 절대 넣지 마세요.** 제목·목차·소제목·Q&A·표 전부.
- 독자의 '이전 번호' 미련은 이렇게 우회: "연락처·사진·카톡 데이터는 단말기에 그대로 남아 있어요" (이 문장에 '번호' 단어 넣지 말 것).${
          isSecondPhoneKeyword
            ? `
- 이 글은 **"두 번째 회선을 새로 개통"**하는 글입니다. 기존 회선은 이 글의 주제가 아닙니다.
  다음은 **절대 금지** (2026-07-28: 이걸 어겨서 3회 연속 폐기됨):
  ✗ **"번호이동"이라는 단어 자체를 쓰지 마세요.** 이 글은 신규 개통 글입니다.
  ✗ 기존 번호/기존 회선이 어떻게 되는지 설명하지 마세요 — 본문·Q&A·표 전부.
  ✗ **Q&A에 번호 관련 질문을 아예 만들지 마세요.** (요금·개통시간·필요서류 등 다른 걸 물으세요)
  → 번호는 "**새 번호를 하나 더 받습니다**" 한 줄로만 언급하고 더 설명하지 마세요.`
            : `
- **Q&A에 번호 관련 질문을 아예 만들지 마세요.** (요금·개통시간·필요서류·유심 종류 등 다른 걸 물으세요)
  → "번호는 어떻게 되나요?" 같은 질문을 만들면 답변에서 거의 매번 "기존 번호를 유지할 수 없어요"가 나와
    글이 통째로 폐기됩니다 (2026-08-04: '통신사직권해지' 3/3 폐기). 질문 자체를 만들지 않는 게 안전합니다.`
        }
...
## 🚫 한국어 AI티·번역투 금지 (위반 시 그 문장을 다시 쓸 것)
- "~에 대해(서)/~에 있어(서)/~를 통해" 남발 금지 → 조사로 직결 ("요금제에 대해 알아보면" ✗ → "요금제를 알아보면")
- "가지고 있다" have 직역 금지 → "장점을 가지고 있어요" ✗ → "장점이 있어요"
- 이중 피동 "~되어진다/~지게 된다" 금지, "~에 의해" 피동 → 행위자를 주어로 ("통신사에 의해 정지된" → "통신사가 정지한")
- "~할 수 있어요" 남발 금지 → 사실은 단언 ("아낄 수 있어요" → "아껴요"). 단, 개통 가능/불가 같은 '가능 여부' 사실 서술은 유지.
- "~한 것입니다/~다는 것입니다" 종결, 추측 종결("~로 보입니다") 남발 금지 → 직접 종결
- 연결어미(-고/-며/-지만) 바로 뒤 쉼표 금지 ("확인하고, 신청하면" ✗ → "확인하고 신청하면")
- 문두 접속사(또한/따라서/즉/나아가) 5회 이상 금지 → 대부분 삭제하고 문장 흐름으로
- "그/그녀/그것" 대명사 직역 금지 → 생략하거나 명사 반복으로
...
14. **단계별 가이드** — 차근차근, 순서대로, N단계
    (※ "사진 N장"처럼 글에 실제로 없는 요소(사진·영상·다운로드 등)를 약속하는 제목 금지 — 본문과 불일치)
    예) \`복잡해 보이는 셀프 개통, 5단계로 순서대로 정리\`
...
export async function generatePost(opts: {
  ...
  // 화자 시점 가드 — 업체 계정이 "저는 미납으로 거절당했어요" 식 1인칭 피해담을
  // 쓰면 폐기. Threads 초안엔 2026-07-20부터 걸려 있었는데 블로그엔 없어서
  // 같은 문장이 그대로 발행됐다 (실측 4편). 위 프롬프트 "화자 시점 규칙"과 짝.
  const victimClaims = findFirstPersonVictimClaims(guardTarget);
  if (victimClaims.length > 0) {
    throw new Error(
      `페르소나 가드: 1인칭 피해 경험담 — 우리는 개통을 도와주는 판매점이지 피해 당사자가 아니다. ` +
        `위반 구간: ${victimClaims.map((x) => `"…${x}…"`).join(" / ")} ` +
        `← "저는/저도/저처럼 …당했어요"를 손님 사례 시점("오신 손님도 …하셨는데 개통해 드렸어요")으로 다시 쓸 것`,
    );
  }
