...
⚠️ **반복 클리셰 절대 금지:**
- ❌ \`5분 비대면 개통\` (지난 글들에 너무 자주 반복됨)
- ❌ \`OK!\` \`끝!\` 같은 평범한 결말 어미
- ❌ \`완벽 가이드\` \`총정리\` 같은 무의미한 SEO 단어

✅ **후킹 패턴 (다음 중 1개 의무 사용):**
1. **구체적 숫자 + 결과**: \`30초에\`, \`월 5만→1만원\`, \`3년 절약\`, \`5가지 차이\`
   예) \`청주 선불폰, 매장 3년 안 가는 사람들이 쓰는 방법\`
2. **페인포인트 + 해결**: 사용자의 진짜 고민을 콕 집어서
   예) \`통신사 거절당해서 폰 못 만들었다면, 이 방법 모르셨군요\`
3. **부정/긴급 후크**: \`절대 ~ 마세요\`, \`이거 모르면 손해\`
   예) \`충주 선불폰 개통 전, 이거 모르면 30분 더 걸립니다\`
4. **호기심 갭(open loop)**: \`직원도 모르는\`, \`알려주지 않는\`
   예) \`통신사 직원도 잘 모르는 미성년자 알뜰폰 가입법\`
5. **질문형 (구체적)**: \`왜 ~ 일까?\`, \`~ 가능할까?\`
   예) \`직권해지 당한 사람, 다른 통신사 갈 수 있을까?\`
6. **타겟 호명 + 상황**: \`{타겟}이라면\`, \`{직업} 위해\`
   예) \`외국인등록증만 있어도, 한국에서 폰 만드는 법\`
7. **비교/대조**: \`A vs B\`, \`~보다 빠른\`
   예) \`KT 바로유심 vs LG 모두의유심, 1분이면 끝나는 비교\`

✅ **길이 25-45자** (검색결과에 잘리지 않음, 모바일에서도 한눈에)
✅ **주 키워드는 제목 시작 부분**에 배치 (SEO)
✅ **강한 동사/형용사**: 끝낸다, 정리, 공개, 노하우, 비법, 진짜, 의외의, 충격, 알려드림
✅ **숫자는 가능하면 구체적으로**: "3분" > "빠른", "5만원" > "절약"

❌ **금지 예시들 (지금까지 제목들):**
- "충주선불폰 5분 비대면 셀프개통 | 앤텔레콤 안심개통" — 클리셰 + 브랜드 접미사
- "선불폰 eSIM, 5분 비대면 개통! 앤텔레콤 안심개통" — 클리셰 + 브랜드
- "미성년자알뜰폰: 5분 비대면 개통, 신용 없이! | 앤텔레콤 안심개통" — 모두 위반

✅ **개선 예시들 (위 키워드들로 다시 만들면):**
- "충주에서 선불폰 만들기, 30분 → 3분으로 줄이는 법"
- "선불폰도 eSIM 됩니다, 유심 없이 5분 안에"
- "신용조회 안 하는 미성년자 알뜰폰, 부모 동의 없이도 가능?"
