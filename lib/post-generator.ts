...
## 🚨 번호 관련 사실 프레임 강제 (이 키워드는 오정보 위험이 높음 — 위반 시 글 전체 폐기)
- 미납·정지·직권해지 상태에서는 **기존 번호를 유지·복구할 수 없습니다. 새 번호가 발급됩니다** (단말기만 그대로 사용).
- "번호 그대로", "번호 유지", "쓰던 번호", "번호 살리기/살릴 수" 표현은 부정문·질문·희망 묘사 포함 **어떤 형태로도 금지**.
- 검색 의도가 '번호 살리기'여도 정답 프레임은 이것입니다: **"기존 번호는 살릴 수 없지만, 새 번호로 5분 만에 다시 개통할 수 있다 (단말기·카톡 데이터는 그대로)"**
- 제목·본문·Q&A·썸네일 카피 전부 이 프레임으로 작성하세요.`
    : "";
